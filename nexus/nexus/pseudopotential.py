...
from .basisset import GaussianBasisSet, process_gaussian_text
from .developer import DevBase, error, log, obj, unavailable, warn
...
from .utilities import is_valid_filename, path_string
        ...
        for code, pseudo_files in code_pps.items():
            ...
            for filename in pseudo_files:
                if pseudo_dir is not None:
                    pseudos.append(Path(pseudo_dir).resolve() / filename)
                else:
...
def ppset(
    ...
    *,
    pseudo_dir: PathLike | None = None,
    **code_pps: Iterable[str],
    ) -> None:
    """Backwards-compatible interface for the legacy ``ppset`` function.
    
    Parameters
    ----------
    label : str
        Label to identify the pseudos
    pseudo_dir : PathLike, optional
        Directory containing the pseudopotentials. If not supplied, this
        will use the ``pseudo_dir`` specified in ``settings``.
    **code_pps
        Dictionary with keys being one or more of the codes supported by
        ``PseudoSet`` and values being lists of pseudopotential file
        names.

    See Also
    --------
    PseudoSet.known_codes : Codes known by Nexus.
    PseudoSet.from_dir : Function to automatically get all pseudos from a directory.
    PseudoSet.from_mixed_dir : Function to automatically organize all pseudos from a directory.
    """
    PseudoSet._register_legacy_ppset(
        label        = label,
        code_pps     = code_pps,
        pseudo_dir   = pseudo_dir,
        )
